...
export interface DAProofsGateway {
  getDaPublication(
    txId: string
  ): Promise<DAStructurePublication<DAEventType, PublicationTypedData>>;
  getTimestampProofs(txId: string): Promise<DATimestampProofsResponse>;
  getBlockRange(blockNumbers: number[], ethereumNode: EthereumNode): Promise<BlockInfo>;
}

export class DAProofChecker {
  constructor(private verifier: DAProofsVerifier) {}
